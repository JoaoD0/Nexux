...
BEGIN
  -- Find and lock an opponent atomically
  ...
  -- Mark the current player as matched
  UPDATE queue SET status = 'matched'
  ...
  v_server_ip := (floor(random()*255)+1)::text || '.' || (floor(random()*255)+1)::text || '.' || (floor(random()*255)+1)::text || '.' || (floor(random()*255)+1)::text || ':27015';
